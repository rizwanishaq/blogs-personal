---
title: "Mastering gRPC with Node.js: Unary RPC"
description: "Explore the intricacies of Unary RPC in gRPC, a high-performance, open-source Remote Procedure Call (RPC) framework. Learn how to efficiently connect services, enable load balancing, tracing, and authentication using HTTP/2 and Protocol Buffers."
...
- `service SquareService`: This defines our service, which contains a single RPC method named square.
...
function square(call, callback) {
  ...
  const response = { number: squareNumber };
  const error = null;
...
server.addService(squarePackage.SquareService.service, { square });
...
- The `square` function is where the magic happens. It takes in a gRPC call object, processes the request, and prepares the response.
...
```javascript
/**
 * Client for making a Unary RPC request to calculate the square of a number.
 */
...
/**
 * Sends a request to the server to calculate the square of a number.
 * @param {Object} request - The request object.
 * @param {Function} callback - Callback function to handle the response.
 */
function requestSquare(request, callback) {
  client.square(request, callback);
}

// Make the request and handle the response
requestSquare(request, (err, response) => {
  if (!err) {
...
- A request object is prepared, specifying the number for which we want to calculate the square.
- The `requestSquare` function initiates the RPC call to the server, passing the request object.
- We define a callback function to handle the server's response.
...
And there you have it! We've taken a deep dive into the world of Unary RPC with gRPC in Node.js. This powerful mode enables efficient communication between services, making it a cornerstone in modern distributed systems.
